...
open(f, "w") do s
  ...
  # loop through all the data vectors to write the corresponding element
  for d in data
  ...
  write(s, "end_header\n")
  # now loop through again, writing the actual data
end
...
open(f, "a") do s
  ...
  ss = stream(s)
  println(typeof(ss))
  # write_ascii_element2(s, d1)
  for d in data[1]
    write_ascii_element(ss, d)
  end
end
